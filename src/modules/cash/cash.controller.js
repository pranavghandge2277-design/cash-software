...
export const createOpeningBalance = async (req, reply) => {
    try {
        const body = req.body;
        ...
        const [rows] = await db.query("SELECT * FROM cash_balance WHERE date = ?", [today]);
        if (rows.length > 0) {
        ...
        // 👉 Insert
        await db.query(`INSERT INTO cash_balance 
